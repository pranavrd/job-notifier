...
function workdayPostedAt(postedOn, now) {
...
function oraclePostedAt(dateStr, now) {
  const t = Date.parse(dateStr);
  if (isNaN(t)) return 0;
  const DAY = 24 * 3600 * 1000;
  let daysAgo = Math.round((Math.floor(now / DAY) - Math.floor(t / DAY)));
...
async function fromOracle({ name, host, siteNumber }) {
  ...
  if (!res.ok) throw new Error(`${res.status} oracle/${name}`);
  const data = await res.json();
  ...
  return list.map((j) =>
    normalize({
      company: name,
      title: j.Title,
      url: `https://${host}/hcmUI/CandidateExperience/en/sites/${siteNumber}/job/${j.Id}`,
...
export async function fetchAllJobs() {
  const tasks = [
    ...
    ...WORKDAY.map((c) => fromWorkday(c)),
    ...ORACLE.map((c) => fromOracle(c)),
    ...AGGREGATORS.map((a) =>
