...
// ROLES / POSITIONS are defined once in lib/config.js and re-exported here so
// existing imports (`from "./classify.js"`) keep working.
export { ROLES, POSITIONS } from "./config.js";
...
// Seniority markers — this board targets early-career/IC roles, so anything
// senior, lead, or managerial is dropped.
const SENIOR = /(\bsenior\b|\bsnr\b|\bsr\.?\b|\bstaff\b|\bprincipal\b|\blead(?:er|s)?\b|\bmanager\b|\bmgr\b|\bdirector\b|\bhead of\b|\bvp\b|vice president|distinguished|\bfellow\b|architect)/;

export function classifyRole(title = "") {
  const t = ` ${title.toLowerCase()} `;

  if (SENIOR.test(t)) return null;   // senior / lead / manager level — skip
